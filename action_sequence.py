...
for e in range(n_episodes):
    ...
    for i in range(len(wifi_loc_time)):
        ...
        # ver. 1
        action_history, coordinate, radius = rl_rnn.forward_5_rounds(wifi_loc_time[i,:520], coordinate, radius, 1)
        # ver. 2
        ...
        if batch_size != 1:
            ...
            if batch_iter != batch_size -1:
                ...
                loss = rl_rnn.back_propagation(action_history_batch, action_label_batch, 1)
                total_loss[e] += loss
        else:
            loss = rl_rnn.back_propagation(action_history, np.array(right_action_set), 1)
            total_loss[e] += loss
    ...
    print("Epoch -", e, round_idx,"avg loss :", total_loss[e], ", lr is :%.6f"%(lambda1(e)), ", avg distance:", avg_distance[e], "\navg IoW:", avg_iow[e], ", avg cont_score:", avg_conti_score[e])
